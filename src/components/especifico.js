...
import Styles from './css/cssEspecifico'
...
class Especifico extends Component {
    ...
    listarServ(){
      firebase.database().ref(`/servicos/especifico`).on('value',(snap) =>{
        var contador = 0;
        snap.forEach((child) => {
          items.push({
            id: child.key,
        ...
        <ImageBackground source={require('./imgs/main.jpg')} style={{width:screenWidth,height:screenHeight}}>
          <View style={{flex:0.14, paddingBottom:8, flexDirection:'row', borderBottomColor:'rgba(255,255,255,0.1)', borderBottomWidth:1}}>
            ...
            <View style={{flex:0.88,borderColor:'white',flexDirection:'column', justifyContent:'center'}}>
              <Text style={{textAlign:'center' , fontSize: 25 , color:'white', fontWeight: 'bold'}}>Especificos</Text>
            </View>
          </View>
          ...
          <View style={{marginTop:8,width:'90%',borderColor:'white'}}>
            ...
            <Prompt
                title="Informe qual o item/software:"
                placeholder="PC Balcao, Servidor ..."
                visible={this.state.promptVisible}
                onCancel={() => {this.setState({promptVisible: false});}}
                onSubmit={(value) => {this.setState({promptVisible: false});
                  Service.equip = value+'<br/>';
                  this.buttonOK(value);
                  this.props.Services(Service,0,0,0,1);
...
export default connect(mapStateToProps, { Services, OrdemServ })(Especifico);